...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Leaf transformation pipeline supporting image or directory modes"
    )
    ...
    if mode == "directory":
        print("⚠ Directory mode selected, but only single-image processing is implemented.")
    ...
    try:
        area = cv.cv2.contourArea(contour)
    except:
        area = 0
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "image_transformations.png"), dpi=200)
