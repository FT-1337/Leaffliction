...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Leaf transformation pipeline supporting image or "
                    "directory modes"
    )
    ...
    parser.add_argument(
        "pos_image",
        nargs="?",
        help="Input leaf image (positional)"
    )
    ...
    parser.add_argument(
        "--outdir",
        "-o",
        help="Output directory for single-image mode (default: ./output)"
    )
    ...
    parser.add_argument(
        "--mask",
        action="store_true",
        help="Enable mask preprocessing when using --src"
    )

    args = parser.parse_args()
    if args.src:
        if not args.dst:
            parser.error("Directory mode requires --dst")
        mode = "directory"
        output_dir = args.dst
        image_path = None

    else:
        image_path = args.image if args.image else args.pos_image

        if not image_path:
            parser.error("Please provide an image (positional or --image).")

        output_dir = args.outdir if args.outdir else "output"
        mode = "single"
    ...
    if mode == "directory":
        print("⚠ Directory mode selected, but only single-image "
              "processing is implemented.")
        print("Processing the FIRST image found...")
    ...
    cv2.imwrite(f"{output_dir}/{filename}_01_original.png", img)
    ...
    cv2.imwrite(f"{output_dir}/{filename}_02_gaussian_blur.png", blurred)
    try:
    ...
    cv2.imwrite(f"{output_dir}/{filename}_03_mask.png", masked)
    try:
        cnts, _ = cv2.findContours(
            mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        contour = max(cnts, key=cv2.contourArea)
    except Exception:
        contour = np.array([[[10, 10]], [[200, 10]],
                            [[200, 200]], [[10, 200]]])
    ...
    cv2.imwrite(f"{output_dir}/{filename}_04_roi_objects.png", roi_vis)
    shape_img = img.copy()
    ...
    cv2.imwrite(f"{output_dir}/{filename}_05_analyze_object.png", shape_img)
    ...
    try:
        ...
        cv2.line(pseudo_img, (minx, cy_center), (maxx, cy_center),
                 (0, 0, 255), 4)

    except Exception:
        pass

    cv2.imwrite(f"{output_dir}/{filename}_06_pseudolandmarks.png", pseudo_img)
    ...
    imgs = [safe_read(os.path.join(output_dir, n)) for n in names]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "image_transformations.png"),
                dpi=200)
    ...
    for name, channel in color_spaces.items():
        hist, bins = np.histogram(
            channel.flatten(), bins=256, range=[0, 256], density=True)
