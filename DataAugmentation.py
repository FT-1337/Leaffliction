...
class DataAugmentor:
    ...
    AUGMENTATION_TYPES = ['Flip', 'Rotate',
                          'Skew', 'Shear', 'Crop', 'Distortion']
    ...
    def __init__(self, path):
        ...
        Args:
            path (str): Path to the directory containing plant images or a
                single image file
        ...
        if self.is_single_file:
            ...
            if not self.data_path.is_dir():
                raise NotADirectoryError(
                    f"Path is not a directory or supported image file: {path}")
    ...
    def get_all_images(self):
        ...
        Returns:
            dict: Dictionary with subdirectory names as keys and lists of
    ...
    def skew_image(self, image):
        ...
        return image.transform(
            image.size, Image.PERSPECTIVE, coeffs, Image.BICUBIC)
    ...
    def run_augmentation(self):
        ...
        for category, image_paths in sorted(images_by_category.items()):
            ...
            if self.is_single_file:
                print(
                    f"  📊 1 image augmented → {augmented_count} new images "
                    "created")
            else:
                print(
                    f"  📊 {category_count} images augmented → "
                    f"{augmented_count} new images created")
        ...
        print(
            f"Total images after augmentation: "
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "\n❌ Usage: python DataAugmentation.py "
